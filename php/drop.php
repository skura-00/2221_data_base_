...
<body>
    ...
    try {
        $sql = "DROP TABLE Student;"."DROP TABLE Member;";
        $stmnt = $conn->prepare($sql);     // read about prepared statements here: https://www.w3schools.com/php/php_mysql_prepared_statements.asp
        $stmnt->bindParam(':stdId', $_POST['StudentID']);   // :stdId is the variable that we used in $sql, there must be a colon (:) in front of it.
        ...
        $stmnt->execute();
        echo "<p style='color:green'>Droped Successfully</p>";
    } catch (PDOException $err) {
        echo "<p style='color:red'>Drop table Failed: " . $err->getMessage() . "</p>\r\n";
    }
